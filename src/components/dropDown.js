...
const DropDown = React.forwardRef(
  ({ options, updateTags, udpateActionsView }, ref) => {
      ...
      <div ref={dropdownRef} className="dropdown">
        ...
        >
          <div className="common-select">
            <div className="tagTo">Tag To :</div>
            <div className="tagged-list">
              {tagged.map((taggedPerson) => (
                <div
                  onClick={(e) => handleClick(e, taggedPerson)}
                  className="highlighted-text tagged-person"
                >
                  {taggedPerson} {'\u2A2F'}
                </div>
              ))}
            ...
            {options.map((option) => {
              return (
                ...
                >
                  <input
                    key={option.name}
                    checked={tagged.includes(option.name)}
                    className="checkbox-custom"
                    type="checkbox"
                  ></input>
                  <div className="option-item" value={option.name}>
                    {option.name}
                  </div>
                </div>
              );
